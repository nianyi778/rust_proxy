...
- [测试与排障](#测试与排障)
- [工作原理](#工作原理)
- [Linux 部署（systemd）](#linux-部署systemd)
- [反向代理（Nginx 示例）](#反向代理nginx-示例)
- [去广告](#去广告)
- [环境变量](#环境变量)
- [安全提示](#安全提示)
- [CI/CD（自动构建与发版）](#cicd自动构建与发版)
- [对外发布素材](#对外发布素材)
...
- 去广告：自动过滤第三方采集站插入 m3u8 的插片广告（详见 [去广告](#去广告)）

## 接口

- `GET /stream?url=...`：主入口
- `GET /api/proxy/stream?url=...`：兼容旧入口
- `GET /health`：健康检查
...
重写 m3u8 时会顺带过滤第三方采集站插入的**插片广告**（SSAI）。识别分三层，只删能高置信判定为广告的分片，**不动正片**：

1. **标准广告标记**：丢弃 `#EXT-X-CUE-OUT` 与 `#EXT-X-CUE-IN` 之间的内容。
2. **路径偏离**：广告分片常来自与正片不同的主机/路径（占比 < 主流 40%）。这类被不连续标记隔开的「异类」区段会被整段删除（含片尾贴片）。
3. **邻居异常**：无路径信号时，若某个短区段（≤ 阈值秒）两侧都夹着远大于它的正片区段，则判为单段广告并删除。对「正片本身被均匀切成大量短段」的打包器（每段都不突出）不会误删。

删除广告后会在拼接处补一个 `#EXT-X-DISCONTINUITY` 让播放器重置解码器，并保留 `#EXT-X-ENDLIST`。

`scripts/analyze_hls_ads.sh` 可用来对某个采集站 API 抽样多个视频、查看其广告特征（部署机/VPS 上运行）。

## 环境变量

- `LISTEN_ADDR`：监听地址，默认 `0.0.0.0:8080`
- `AD_FILTER_MAX_SECS`：去广告「邻居异常」规则的区段时长上限（秒），默认 `30`。设为 `0` 可关闭该规则（仍保留标记/路径两层过滤）。
...
如果你要对外发布（掘金长文 / 推特线程 / 个人博客版），已整理好可复制的 Markdown：

- [docs/press/README.md](docs/press/README.md)
